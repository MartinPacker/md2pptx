...
def createTableBlock(slideInfo, slide, renderingRectangle, tableBlockNumber):
    ...
    if isGraphicsGrid == True:
        ...
        # Potentially add a caption row
        if tableCaptions[tableBlockNumber] == "":
            captionRowCount = 0
        else:
            captionRowCount = 1
            
            tableRows.append([tableCaptions[tableBlockNumber]])
            rows += 1
        ...
        rowCountBlurb =str(rows - captionRowCount)
        ...
        for rowNumber, row in enumerate(tableRows):
            ...
            for columnNumber, cell in enumerate(row):
                ...
                # Massage any table caption cell
                if (captionRowCount == 1) & (rowNumber == rows -1) & (columnNumber == 0):
                    if numberedTableCaptions:
                        cell = f"*Table {globals.tableNumber}. {cell}*"
...
if (inBlock is True) | (inCode is True) | (inTable is True):
    slideInfo = SlideInfo(
        slideTitle, slideSubtitle, blockType, bullets, tableRows, cards, code, sequence, tableCaptions
    )
    slideNumber, slide, sequence = createSlide(prs, slideNumber, slideInfo)
