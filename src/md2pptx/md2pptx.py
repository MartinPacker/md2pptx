...
def showRenderingRectangle(slide, renderingRectangle):
    rr = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE,
    ...
    rr.fill.background()
    rr.line.color.rgb = RGBColor(255, 0, 0) 
...
class SlideInfo:
    def __init__(
        ...
        self.tableCaptions = tableCaptions
        self.figureCaptions = figureCaptions
...
def addMissingMediaPlaceholder(slide, mediaFilename, left, top, width, height):
    # adjust position and size of rectangle
    borderFactor = 0.1
    adjustedLeft = int(left + borderFactor * width)
    adjustedTop = int(top +borderFactor * height)
    adjustedWidth = int(width - 2 * borderFactor * width)
    adjustedHeight = int(height - 2 * borderFactor * height)    

    # Create the rectangle the same size as the media would've fitted into - minus a border
    missingRectangle = slide.shapes.add_shape (MSO_SHAPE.RECTANGLE,adjustedLeft, adjustedTop, adjustedWidth, adjustedHeight)
    
    # Label the rectangle with a suitable message, centred
    missingRectangle.text = "Missing Media: " + mediaFilename
    missingRectangle.text_frame.paragraphs[0].alignment = PP_ALIGN.CENTER
    missingRectangle.text_frame.paragraphs[0].font.size = Pt(10)
    
    return missingRectangle

def parseColorStyle(whichStyleType, name, value, colorList):
    spanClass = name[len("style." + whichStyleType) + 1 :]
...
def getGraphicDimensions(fname):
    ...
    try:
        with open(fname, "rb") as fhandle:
            ...
            elif fname.endswith((".jpeg", ".jpg")) | (head[0:3] == b'\xFF\xD8\xFF'):
                try:
...
def renderText(shape, bulletList):
    ...
    shape.text = ""
    for bulletNumber, bullet in enumerate(bulletList):
        ...
        # Unpack bullet information
        level, textToFormat, bulletType = bullet
        ...
        addFormattedText(p, textToFormat)
...
def addFooter(presentation, slideNumber, slide):
    ...
    numbersfontsizespec = globals.processingOptions.getCurrentOption("numbersFontSize")
...
def createContentSlide(presentation, slideNumber, slideInfo):
    ...
    if (slideInfo.bullets == []) & (slideInfo.cards == []):
        # No bullets or cards so "title only" layout wanted
        slideLayout = titleOnlyLayout
        haveBulletsCards = False
    else:
        # Either bullets or cards or both so "content slide" layout wanted
    ...
    for b in range(blocksToRender):
        ...
        if slideInfo.sequence[b] == "table":
            ...
            createListBlock(slideInfo, slide, renderingRectangle, listBlockNumber)
            listBlockNumber += 1
        else:
...
def createListBlock(slideInfo, slide, renderingRectangle, listBlockNumber):
    ...
    if listBlockNumber > 0:
        bulletsShape = addClonedShape(slide, bulletsShape)
    ...
    if len(slideInfo.bullets) == 0:
        bulletCount = 0
    else:
        bulletCount = len(slideInfo.bullets[listBlockNumber])
    ...
    if slideInfo.cards == []:
        ...
        for c, card in enumerate(slideInfo.cards):
            ...
            card.missingCardGraphic = False
            card.missingCardMedia = False
            ...
            if card.graphic != "":
                ...
                try:
                    ...
                    # try to create card graphic shape - to be resized later
                    ...
                    card.missingCardGraphic = True
                    
                    card.graphicShape = addMissingMediaPlaceholder(slide,card.printableFilename, 0, 0, 0, 0)

                    print(
                        "Missing card image file: "
                        + card.printableFilename
                    )
                ...
                try:
                    ...
                    # Try to create card video shape - to be resized later
                    ...
                    card.missingCardMedia = True

                    card.mediaShape = addMissingMediaPlaceholder(slide,card.printableFilename,0, 0, 0, 0)

                    print(
                        "Missing card media file: "
                        + card.printableFilename
                    )
    ...
    if len(slideInfo.bullets) > 0:
        renderText(bulletsShape, slideInfo.bullets[listBlockNumber])

    # Second go on each card
    for c, card in enumerate(slideInfo.cards):
        # Get the shapes for this card - including any graphic or other media
        ...
        if (cardGraphicShape == None) & (cardMediaShape == None):
            ...
            if card.missingCardMedia | card.missingCardGraphic:
                cardMediaWidth = cardGraphicSizeRaw
                cardMediaHeight = cardGraphicSizeRaw
            ...
            if cardGraphicPosition == "before":
                ...
                if cardLayout == "horizontal":
                    ...
                    if cardMediaShape is not None:
                        ...
                        cardGraphicShape.top = cardBodyShape.top + \
                            int((cardBodyHeight - cardMediaHeight) / 2)
                ...
                if cardLayout == "horizontal":
                    if cardMediaShape is not None:
                        ...
                        cardGraphicShape.top = cardBodyShape.height + cardBodyShape.top + \
                            cardGraphicPadding
            ...
            if cardLayout == "horizontal":
                if cardMediaShape is not None:
                    ...
                    cardGraphicShape.left = cardTitleShape.left + \
                        int((cardTitleShape.width - cardMediaWidth) / 2)
                ...
                if cardGraphicPosition == "before":
                    if cardMediaShape is not None:
                        ...
                        cardGraphicShape.left = card.left + cardGraphicPadding
                    ...
                    if cardMediaShape is not None:
                        ...
                        cardGraphicShape.left = card.left + cardBodyShape.width + \
                            cardGraphicPadding

            # Set dimensions of card graphic or media shape
            if cardMediaShape is not None:
                cardMediaShape.width = int(cardMediaWidth)
                cardMediaShape.height = int(cardMediaHeight)

            else:
                cardGraphicShape.width = int(cardMediaWidth)
                cardGraphicShape.height = int(cardMediaHeight)
...
def createTableBlock(slideInfo, slide, renderingRectangle, tableBlockNumber):
    ...
    if isGraphicsGrid == True:
        ...
        ####################################################################
        topLeftMediaMissing = False
        topRightMediaMissing = False
        bottomLeftMediaMissing = False
        bottomRightMediaMissing = False
        leftMediaMissing = False
        rightMediaMissing = False
        ...
        figCaps = slideInfo.figureCaptions
        figCapCount = len(figCaps)

        figCapHeight = Inches(0.3)
        
        topLeftFigCapHeight = figCapHeight if figureNumber < figCapCount else 0
        
        if (topLeftGraphicFilename != "") | (topLeftVideo is not None):
            # Establish dimensions, taking into account any <figcaption>
            (
                topLeftPicWidth,
                topLeftPicHeight,
                usingHeightToScale,
                ...
                maxPicWidth,
                maxPicHeight - topLeftFigCapHeight,
                topLeftMediaWidth,
                topLeftMediaHeight
            )
            ...
            # Calculate vertical start - centred within the rendering rectangle
            topLeftPicY = renderingRectangle.top + (maxPicHeight - topLeftPicHeight - topLeftFigCapHeight) / 2

            if gridRows == 2:
                topLeftPicY -= Inches(0.2)

        if topLeftGraphicFilename != "":
            if leftMediaMissing or topLeftMediaMissing:
                addMissingMediaPlaceholder(
                    slide,
                    printableTopLeftGraphicFilename,
                    renderingRectangle.left,
                    renderingRectangle.top,
                    maxPicWidth,
                    maxPicHeight - Inches(0.2)
                )
                ...
                if len(figureCaptions) > 0:
                    topLeftFigCap = slide.shapes.add_textbox(topLeftPicX,
                        renderingRectangle.bottom - topLeftFigCapHeight,
                        topLeftPicWidth,
                        topLeftFigCapHeight
                    )
                    topLeftFigCapTF = topLeftFigCap.text_frame
                    topLeftFigCapP = topLeftFigCapTF.paragraphs[0]
                    addFormattedText(topLeftFigCapP, f"*{figureCaptions[-1]}")
                    
                    topLeftFigCapP.font.size = Pt(16)
        ...
        if gridColumns == 2:
            ...
            if topRightGraphicFilename != "":
                if rightMediaMissing or topRightMediaMissing:
                    addMissingMediaPlaceholder(
                        slide,
                        printableTopRightGraphicFilename,
                        midGraphicsSpaceX,
        ...
        if gridRows == 2:
            ...
            if bottomLeftGraphicFilename != "":
                if bottomLeftMediaMissing:
                    addMissingMediaPlaceholder(
                        slide,
                        printableBottomLeftGraphicFilename,
                        renderingRectangle.left,
                        renderingRectangle.top + renderingRectangle.height - maxPicHeight + Inches(0.2),
                        maxPicWidth,
                        maxPicHeight - Inches(0.2)
                    )
        ...
        # Potentially add a caption
        try:
            tableHasCaption = (tableCaptions[tableBlockNumber] != "")
        except:
            tableHasCaption = False

        tableCaptionAtTop = globals.processingOptions.getCurrentOption("tableCaptionAtTop")
        tableCaptionHeight = Inches(globals.processingOptions.getCurrentOption("tableCaptionHeight"))
        tableCaptionAlignment = globals.processingOptions.getCurrentOption("tableCaptionAlignment")

        if tableHasCaption:
            tableCaptionAtBottom = not tableCaptionAtTop
            
            if tableCaptionAtBottom:
                # Add another row for the caption
                rows += 1
        
            
        # Create the table with the above number of rows and columns
        newTableShape = slide.shapes.add_table(
            rows,
            columns,
            0,
            0,
        ...
        if tableHasCaption & tableCaptionAtTop:
            newTableShape.top = renderingRectangle.top + tableCaptionHeight
            ...
            newTableShape.height = min(renderingRectangle.height - tableCaptionHeight, Inches(0.25) * rows)
        ...
        if tableHasCaption:
            # Get options table captions
            numberedTableCaptions = globals.processingOptions.getCurrentOption("numberedTableCaptions")

            if numberedTableCaptions:
                captionMarkdown = f"*Table {globals.tableNumber}. {tableCaptions[tableBlockNumber]}*"

                globals.tableNumber += 1
            else:
                captionMarkdown = f"*{tableCaptions[tableBlockNumber]}*"
            
            if tableCaptionAtTop:
                captionShape = slide.shapes.add_textbox(
                    renderingRectangle.left,
                    renderingRectangle.top,
                    renderingRectangle.width,
                    tableCaptionHeight
                ...
                p = captionShape.text_frame.paragraphs[0]

                if tableCaptionAlignment == "r":
                    p.alignment = PP_ALIGN.RIGHT
                elif tableCaptionAlignment == "c":
                    p.alignment = PP_ALIGN.CENTER
                else:
                    p.alignment = PP_ALIGN.LEFT

                addFormattedText(p, captionMarkdown)
        ...
        if tableHasCaption == 0:
            tableCaptionBlurb = ""
        else:
            if tableHeadingBlurb == "without heading":
                tableCaptionBlurb = "with caption"
            else:
               tableCaptionBlurb = " and caption"

        rowCountBlurb =str(rows - tableHasCaption)
        ...
        for rowNumber, row in enumerate(tableRows):
            ...
            for columnNumber, cell in enumerate(row):
                ...
                # Add the cell text, respecting formatting
                addFormattedText(p, cell)
                
                # Potentially colour cell background
                firstSpanClassWithCellColor = None

                for spanClassMatch in globals.spanClassRegex.finditer(cell):
                    # There are span classes that might feature in cellcolors
                    afterSpanTag = cell[spanClassMatch.end(1) :]
                    className = afterSpanTag[7 : afterSpanTag.index(">") - 1]
                    if className in globals.cellcolors:
                        # First <span> of cell text specifies cellcolor
                        firstSpanClassWithCellColor = className
                        
                        # Don't search any further
                        break

                if firstSpanClassWithCellColor is not None:
                    cellBackgroundColor = globals.cellcolors[firstSpanClassWithCellColor]

                    cff = newCell.fill
                    cff.solid()
                    cff.fore_color.rgb = RGBColor.from_string(cellBackgroundColor)
        ...
        # Potentially adjust cell boxes
        for rowNumber, row in enumerate(tableRows):
            for columnNumber, cell in enumerate(row):
                firstSpanClassWithCellBorderStyling = None

                for spanClassMatch in globals.spanClassRegex.finditer(cell):
                    # There are span classes that might feature in cellBorderStyling
                    afterSpanTag = cell[spanClassMatch.end(1) :]
                    className = afterSpanTag[7 : afterSpanTag.index(">") - 1]
                    if className in globals.cellBorderStyling:
                        # First <span> of cell text specifies cellBorderStyling
                        firstSpanClassWithCellBorderStyling = className
                        
                        # Don't search any further
                        break

                if firstSpanClassWithCellBorderStyling is not None:
                    applyCellBorderStyling(
                        newTable.cell(rowNumber, columnNumber),
                        globals.cellBorderStyling[firstSpanClassWithCellBorderStyling],
                        globals.cellLineWidthMultiplier[firstSpanClassWithCellBorderStyling],
                        globals.cellLineCount[firstSpanClassWithCellBorderStyling],
                        globals.cellLineColour[firstSpanClassWithCellBorderStyling],
                    )

        if tableHasCaption:
            if tableCaptionAtBottom:
                captionRow = newTable.rows[len(newTable.rows)-1]
                
                # Following has no effect - so commented out
                # captionRow.height = tableCaptionHeight
                
                captionCell = captionRow.cells[0]
                p = captionCell.text_frame.paragraphs[0]
                ...
                lastCell = captionRow.cells[len(captionRow.cells) - 1]
                lastCell.merge(captionCell)
                
                captionCellFill = captionCell   .fill
                captionCellFill.background()
...
md2pptx_path = os.path.dirname(__file__)
...
print(f"\nmd2pptx is found in {md2pptx_path}")
...
dynamicMetadata2Regex = re.compile("^<!-- md2pptx: (.+): (.+) -->")
dynamicMetadata1Regex = re.compile("^<!-- md2pptx: (.+) -->")
indirectReferenceAnchorRegex = re.compile(r"^\[(.+?)\]: (.+)")
indirectReferenceUsageRegex = re.compile(r"[(.+?)]\[(.+?)]")
figcaptionRegex = re.compile(r"^<figcaption>(.+)</figcaption>$")
...
globals.abbrevDictionary = {}

# Abbreviation Runs Dictionary
globals.abbrevRunsDictionary = {}
...
globals.processingOptions.setOptionValuesArray(
    [
        ...
        ["tableHeadingSize", 0],
        ["tableShadow", False],
        ["numberedTableCaptions", False],
        ["tableCaptionAtTop", False],
        ["tableCaptionHeight", 0.4],
        ["tableCaptionAlignment", "l"],
    ]
)
...
# Scripting emitter options
globals.processingOptions.setOptionValuesArray(
    [
        ["applescript", ""],
        ["applescriptReload", False],
        ["applescriptRun", False],
        ["bookmark", 0],
        ["applescriptPrologue", []],
        ["applescriptEpilogue", []],
    ]
...
for line in metadata_lines:
    ...
    elif name == "numberedtablecaptions":
        if value.lower() == "yes":
            globals.processingOptions.setOptionValues(name, True)

    elif name == "tablecaptionattop":
        if value.lower() == "yes":
            globals.processingOptions.setOptionValues(name, True)

    elif name == "tablecaptionheight":
        globals.processingOptions.setOptionValues(name, float(value))

    elif name == "tablecaptionalignment":
        globals.processingOptions.setOptionValues(name, value[:1])
    ...
    # Following relate to styling and don't use Processing Options class
        ...
        if name.startswith("style.bgcolor."):
            parseColorStyle("bgcolor", name, value, globals.bgcolors)

        elif name.startswith("style.fgcolor."):
            parseColorStyle("fgcolor", name, value, globals.fgcolors)

        elif name.startswith("style.cellcolor."):
            parseColorStyle("cellcolor", name, value, globals.cellcolors)

        elif name.startswith("style.cellbox."):
            spanClass = name[14:]
            cellboxArray = value.split(" ")
            if len(cellboxArray) > 0:
                parseColorStyle("cellbox", name, cellboxArray[0], globals.cellLineColour)
                globals.cellLineWidthMultiplier[spanClass] = 1
                globals.cellLineCount[spanClass] = 1
                globals.cellBorderStyling[spanClass] = "lrtb"
    ...
    elif name == "applescriptoptions":
        for option in value.split():
            if option == "reload":
                globals.processingOptions.setOptionValues("applescriptReload", True)
        
            if option == "run":
                globals.processingOptions.setOptionValues("applescriptRun", True)
        
    elif name == "bookmark":
        globals.processingOptions.setOptionValues(name, int(value))
        
    elif name in ["applescriptprolog", "applescriptprologue"]:
        if value[:1] == "!":
            prologFile = md2pptx_path + os.sep + "applescript" + os.sep + value[1:]
        else:
            prologFile = value

        applescriptPrologueFiles = globals.processingOptions.getCurrentOption("applescriptprologue")
        ...
        applescriptEpilogueFiles = globals.processingOptions.getCurrentOption("applescriptepilogue")
        applescriptEpilogueFiles.append(epilogFile)
        globals.processingOptions.setOptionValues("applescriptepilogue", applescriptEpilogueFiles)
        
# Add any prologue file contents to the AppleScript that will be generated
if (applescriptPrologueFiles := globals.processingOptions.getCurrentOption("applescriptprologue")) is not []:
    for prologueFile in applescriptPrologueFiles:
        try:
            with open(prologueFile, 'r') as pf:
                pfLines = pf.readlines()
                
                for pfLine in pfLines:
                    if pfLine.rstrip("\n") != "":
                        pfLine = pfLine.rstrip("\n")
                    globals.applescriptLines.append(pfLine)
        except IOError:
            print(f"Could not open AppleScript Prologue file {prologueFile}. Ignoring.")
    globals.applescriptLines.append("")

if (applescriptFilename := globals.processingOptions.getCurrentOption("applescript")) != "":
    globals.applescriptLines += [
        "tell application \"Microsoft PowerPoint\"",
        "    set activePresentation to full name of active presentation",
        "    if activePresentation is not missing value",
        "        tell active presentation",
    ]

# Prime AppleScript presentation insert list
ASpresentationInserts = []
...
for line in afterMetadataAndHTML:
    if startswithOneOf(line, ["<pre>", "<code>"]):
        ...
        # This is a figure caption line so keep it separate
        linesAfterConcatenation.append(line)
...
for line in linesAfterConcatenation:
    ...
    if m := indirectReferenceAnchorRegex.match(line):
        ...
        globals.indirectAnchors.append([anchorName, anchorURL])
...
for lineNumber, line in enumerate(linesAfterConcatenation):
    ...
    elif startswithOneOf(line, ["<br/>", "<br />"]):
        inList = False
    ...
    if line[:1] == "-":
        ...
        if DMMatch := dynamicMetadata2Regex.match(line):
            metadataKey = DMMatch.group(1).lower().strip()
            if metadataKey == "bookmark":
                print("hekko")
            ...
            if (metadataKey != "") & (metadataValue != ""):
                ...
                elif metadataKey == "asinsertpresentation":
                    ASinsertAfterSlide = slideNumber

                    ASpresentationInserts.append((ASinsertAfterSlide, metadataValue))

                else:
                    # Invalid dynamic metadata specification
                    print(f"Invalid dynamic metadata key: '{metadataKey}' in '{line}'")

        elif DMMatch := dynamicMetadata1Regex.match(line):
            metadataKey = DMMatch.group(1).lower().strip()

            if metadataKey == "bookmark":
                globals.processingOptions.setOptionValues(metadataKey, slideNumber + 1)
    ...
    elif match := figcaptionRegex.match(line.rstrip()):
        # Extract the figure caption text
        if inTable:
            # Update latest graphic to have a figure caption
            figcap = match.group(1)
            figureCaptions.append(figcap)

    elif match := (bulletRegex.match(line)) or (numberRegex.match(line)):
        if match.re is bulletRegex:
            listItemType = "bulleted"
        else:
            listItemType = "numbered"

        # Is a list item
        listItemLine = match.group(3).lstrip()

        listItemLevel = calculateIndentationLevel(
            match.start(2), globals.processingOptions.getCurrentOption("indentSpaces")
        )
        
        if inCard:
            # Bullet is on a card
            cardBullets.append([listItemLevel, listItemLine, listItemType])
            ...
            if inList is False:
                bulletList = []
                bulletList.append([listItemLevel, listItemLine, listItemType])
                bullets.append(bulletList)
            else:
                bulletList = bullets[-1]
                bulletList.append([listItemLevel, listItemLine, listItemType])
                bullets[-1] = bulletList                
...
#                                                                                    #
######################################################################################
if len(globals.abbrevDictionary) > 0:
    glossaryTermsPerPage = globals.processingOptions.getCurrentOption("glossaryTermsPerPage")
    slideNumber, glossarySlides = createGlossarySlides(
        prs, slideNumber, globals.abbrevDictionary
    )
    # Fix up internal glossary hyperlinks
    abbrevNumber = -1
    for abbreviation in sorted(globals.abbrevRunsDictionary.keys()):
        abbrevNumber += 1
        runs = globals.abbrevRunsDictionary[abbreviation]
        for run in runs:
            # Add tooltip for glossary definition
            glossarySlideNumber = int(abbrevNumber / glossaryTermsPerPage)
            createRunHyperlinkOrTooltip(
                run, glossarySlides[glossarySlideNumber], globals.abbrevDictionary[abbreviation]
            )
...
for (picture, href, tooltip, figcap) in pictureInfos:
...
# Create any AppleScript
if applescriptFilename != "":
    
    want_save = False
    
    want_reload = globals.processingOptions.getCurrentOption("applescriptReload")
    want_run = globals.processingOptions.getCurrentOption("applescriptRun")

    # Maybe add "save presentation" code
    if want_save:
        globals.applescriptLines.append("        save")
    
    if want_reload:
        globals.applescriptLines.append("            close front document window")

    # Close off "active presentation" tell
    globals.applescriptLines += [
        "        end tell",
        "    end if",
        "",
    ...
    if want_reload:
        ...
        globals.applescriptLines+=[
            f"    set targetPresentation to \"{full_filename}\"",
            "    open targetPresentation",
            "",
        ]


    # Maybe add "go to slide" code
    if (bookmark := globals.processingOptions.getCurrentOption("bookmark")) > 0:
        globals.applescriptLines += [
            "    tell active presentation",
            "        set FDWView to view of front document window",
            "        go to slide FDWView number " + str(bookmark),
            "    end tell"
            "",
    ]

    # Close off applescript
    globals.applescriptLines+= [
        "end tell",
        "",
        "tell application \"Microsoft PowerPoint\"",
    ...
    # Handle any instructions to insert slides from other presentations
    if ASpresentationInserts is not []:
        if templateSlideCount > 0:
            slideNumberAdjustment = 1
        else:
            slideNumberAdjustment = 0

        target_path = os.path.dirname(os.path.abspath(output_filename))

        # Plant calls to copySlides() - in reverse order to maintain slide numbers in the target presentation
        for copyInstruction in reversed(ASpresentationInserts):
            slideAfter, sourcePresentation = copyInstruction
            
            slideAfter += slideNumberAdjustment
            
            if sourcePresentation[:1] != os.sep:
                sourcePresentation = target_path + os.sep + sourcePresentation

            globals.applescriptLines.append(f"\ncopySlides(\"{sourcePresentation}\", targetPresentation,  {slideAfter}, \"yes\")")
    
        # Ensure applescript to copy slides from one presentation to another is present
        copyPresentationAS = md2pptx_path + os.sep + "applescript" + os.sep + "copyPresentation.applescript"
        
        applescriptEpilogueFiles.append(copyPresentationAS)

    # Add any epilogue file contents to the AppleScript that will be generated
    if applescriptEpilogueFiles is not []:
        globals.applescriptLines.append("")

        for epilogueFile in applescriptEpilogueFiles:
            try:
                with open(epilogueFile, 'r') as ef:
                    efLines = ef.readlines()
                    
                    for efLine in efLines:
                        if efLine.rstrip("\n") != "":
                            efLine = efLine.rstrip("\n")
                        globals.applescriptLines.append(efLine)
            except IOError:
                print(f"Could not open AppleScript Epilogue file {epilogueFile}. Ignoring.")

    # Write the applescript out
    with open(applescriptFilename, "w") as a:
        for l in globals.applescriptLines:
            a.write(l + "\n")

    if want_run:
        os.system(f"osascript {applescriptFilename}")
